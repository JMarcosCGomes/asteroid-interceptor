...
class Universo:
    ...
    def criar_corpos_celestes(self):
        ...
        # === Mercurio ===
        raio_orbita_mercurio = 5.791e10
        angulo_rad = np.deg2rad(60)
        x_mercurio = raio_orbita_mercurio * np.cos(angulo_rad)
        y_mercurio = raio_orbita_mercurio * np.sin(angulo_rad)
        v_mercurio_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_mercurio)
        vx_mercurio = -v_mercurio_modulo * np.sin(angulo_rad)
        vy_mercurio = v_mercurio_modulo * np.cos(angulo_rad)
        self.Mercurio = Corpo_Celeste(massa=3.301e23,raio=2.44e6,color="sienna",name="Mercurio",pos_x=x_mercurio,pos_y=y_mercurio,vel_x=vx_mercurio,vel_y=vy_mercurio,)
        self.corpos_celestes.append(self.Mercurio)

        # === Venus ===
        raio_orbita_venus = 1.082e11
        angulo_rad = np.deg2rad(30)
        x_venus = raio_orbita_venus * np.cos(angulo_rad)
        y_venus = raio_orbita_venus * np.sin(angulo_rad)
        v_venus_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_venus)
        vx_venus = -v_venus_modulo * np.sin(angulo_rad)
        vy_venus = v_venus_modulo * np.cos(angulo_rad)
        self.Venus = Corpo_Celeste(massa=4.867e24,raio=6.052e6,color="lightyellow",name="Venus",pos_x=x_venus,pos_y=y_venus,vel_x=vx_venus,vel_y=vy_venus,)
        self.corpos_celestes.append(self.Venus)
        ...
        self.Marte = Corpo_Celeste(massa=6.417e23,raio=3.389e6,color="red",name="Marte",pos_x=x_marte,pos_y=y_marte,vel_x=vx_marte,vel_y=vy_marte,)
        self.corpos_celestes.append(self.Marte)

        # === Jupiter ===
        raio_orbita_jupiter = 7.785e11
        angulo_rad = np.deg2rad(10)
        x_jupiter = raio_orbita_jupiter * np.cos(angulo_rad)
        y_jupiter = raio_orbita_jupiter * np.sin(angulo_rad)
        v_jupiter_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_jupiter)
        vx_jupiter = -v_jupiter_modulo * np.sin(angulo_rad)
        vy_jupiter = v_jupiter_modulo * np.cos(angulo_rad)
        self.Jupiter = Corpo_Celeste(massa=1.898e27,raio=6.991e7,color="orange",name="Jupiter",pos_x=x_jupiter,pos_y=y_jupiter,vel_x=vx_jupiter,vel_y=vy_jupiter,)
        self.corpos_celestes.append(self.Jupiter)

        # === Saturno ===
        raio_orbita_saturno = 1.434e12
        angulo_rad = np.deg2rad(30)
        x_saturno = raio_orbita_saturno * np.cos(angulo_rad)
        y_saturno = raio_orbita_saturno * np.sin(angulo_rad)
        v_saturno_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_saturno)
        vx_saturno = -v_saturno_modulo * np.sin(angulo_rad)
        vy_saturno = v_saturno_modulo * np.cos(angulo_rad)
        self.Saturno = Corpo_Celeste(massa=5.683e26,raio=5.823e7,color="gold",name="Saturno",pos_x=x_saturno,pos_y=y_saturno,vel_x=vx_saturno,vel_y=vy_saturno,)
        self.corpos_celestes.append(self.Saturno)

        # === Urano ===
        raio_orbita_urano = 2.873e12
        angulo_rad = np.deg2rad(45)
        x_urano = raio_orbita_urano * np.cos(angulo_rad)
        y_urano = raio_orbita_urano * np.sin(angulo_rad)
        v_urano_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_urano)
        vx_urano = -v_urano_modulo * np.sin(angulo_rad)
        vy_urano = v_urano_modulo * np.cos(angulo_rad)
        self.Urano = Corpo_Celeste(massa=8.681e25,raio=2.536e7,color="lightblue",name="Urano",pos_x=x_urano,pos_y=y_urano,vel_x=vx_urano,vel_y=vy_urano,)
        self.corpos_celestes.append(self.Urano)

        # === Netuno ===
        raio_orbita_netuno = 4.495e12
        angulo_rad = np.deg2rad(60)
        x_netuno = raio_orbita_netuno * np.cos(angulo_rad)
        y_netuno = raio_orbita_netuno * np.sin(angulo_rad)
        v_netuno_modulo = np.sqrt(self.G * self.Sol.massa / raio_orbita_netuno)
        vx_netuno = -v_netuno_modulo * np.sin(angulo_rad)
        vy_netuno = v_netuno_modulo * np.cos(angulo_rad)
        self.Netuno = Corpo_Celeste(massa=1.024e26,raio=2.462e7,color="darkblue",name="Netuno",pos_x=x_netuno,pos_y=y_netuno,vel_x=vx_netuno,vel_y=vy_netuno,)
        self.corpos_celestes.append(self.Netuno)
